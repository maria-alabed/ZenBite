...
function AvailabilityBadge({ availability }) {
  ...
  return (
    <span style={{
      ...
      background:"#fde8e8", border:"1px solid #e74c3c", marginTop:"4px",
    }}>
      ❌ Out of Stock
    </span>
  );
...
export default function MenuPage() {
  ...
  const [selectedCategory, setSelectedCategory] = useState(
    () => location.state?.selectedCategory || "Japanese Kitchen"
  );
  ...
  // ── ترجمة المنتج (سطران فقط بدل 150 سطر) ─────────────
  ...
  const filteredDishes = dishes.filter((dish) => {
    ...
    const dishPrice = dish.sizes
      ...
      : dish.price;
    return matchesSearch && dishPrice >= priceRange.min && dishPrice <= priceRange.max;
  });

  const sortedDishes = [...filteredDishes].sort((a, b) => {
    const pa = a.sizes ? Math.min(...Object.values(a.sizes)) : a.price;
    const pb = b.sizes ? Math.min(...Object.values(b.sizes)) : b.price;
    switch (sortBy) {
      case "price_low":  return pa - pb;
      case "price_high": return pb - pa;
      case "rating":     return parseFloat(b.rating) - parseFloat(a.rating);
      case "name":       return getTranslatedProduct(a).name.localeCompare(getTranslatedProduct(b).name);
      default:           return 0;
    }
  });
  ...
  useEffect(() => {
    ...
    for (const [category, dishes] of Object.entries(menuData)) {
      ...
      if (dish) {
        ...
        applyThemeVars(themes[category]);
        setDrawerProduct({ ...dish, category });
        break;
      }
    }
  }, [location.state]);

  useEffect(() => { if (theme) applyThemeVars(theme); }, [selectedCategory, theme]);
  ...
  const handleCategoryChange = (category) => {
    ...
    applyThemeVars(themes[category]);
    setSearchQuery(""); setPriceRange({ min:0, max:100 }); setSortBy("default");
    setTimeout(() => {
      setSelectedCategory(category);
  ...
  const getCurrentPrice  = (dish) => {
    if (dish.sizes) return dish.sizes[getSelectedSize(dish.id)] ?? dish.sizes.M;
    return dish.price;
  };
  ...
  const handleQuickAdd = (e, dish) => {
    ...
    setTimeout(() => {
      addToCart(isItalian && dish.sizes
        ? { ...dish, category: selectedCategory, selectedSize, price: currentPrice,
            name: `${translatedName} (${selectedSize})`, originalName: dish.name }
        : { ...dish, category: selectedCategory, name: translatedName, originalName: dish.name }
      );
    }, 800);
  };

  const resetFilters = () => { setSearchQuery(""); setPriceRange({ min:0, max:100 }); setSortBy("default"); };

  const avgRating = (dishes.reduce((a,d) => a + parseFloat(d.rating), 0) / dishes.length).toFixed(1);
  const minPrice  = Math.min(...dishes.map(d => d.sizes ? Math.min(...Object.values(d.sizes)) : d.price)).toFixed(2);
  const maxPriceOverall = Math.max(...dishes.map(d => d.sizes ? Math.max(...Object.values(d.sizes)) : d.price));
    ...
    <>
      ...
      <section className={`menu-page ${isTransitioning ? "page-transition" : ""}`}>
        ...
        <div className="menu-stats-row">
          ...
          <span className="menu-stat-chip">🔍 {sortedDishes.length} {t.found}</span>
        </div>
        ...
        <div className="search-filter-bar">
          <div className="search-container">
            ...
            <input
              ...
              placeholder={t.searchForDishes} value={searchQuery}
          ...
          <div className="filter-controls">
            ...
            >
              <Filter size={18} /><span>{t.filters}</span>
            </button>
                ...
                <div className="price-inputs">
                  ...
                  <div className="price-input-group">
                    ...
                    <input type="number" value={priceRange.max}
                      ...
                      min={priceRange.min} max={maxPriceOverall||100} />
                  </div>
                </div>
                <input type="range" min={0} max={maxPriceOverall||100} value={priceRange.max}
                  onChange={(e) => setPriceRange({ ...priceRange, max: Number(e.target.value) })}
                  className="price-slider"
                  style={{ background:`linear-gradient(to right,#ff8a00 0%,#ff8a00 ${(priceRange.max/(maxPriceOverall||100))*100}%,#e0d6ce ${(priceRange.max/(maxPriceOverall||100))*100}%,#e0d6ce 100%)` }}
                />
              </div>
            </div>
            <div className="filter-actions">
              <button className="reset-filters-btn" onClick={resetFilters}>{t.resetFilters}</button>
            </div>
          </div>
        )}
                    ...
                    <div className="dish-info">
                        ...
                        <div className="pizza-size-selector">
                          {["S","M","L"].map((size) => (
                            <button
                              ...
                              onClick={(e) => { e.stopPropagation(); setPizzaSizes(prev => ({ ...prev, [dish.id]: size })); }}
                            >
                              {size} <small>{size==="S" ? t.small : size==="M" ? t.medium : t.large}</small>
                            </button>
                          ))}
                        </div>
                      )}
                      ...
                      <div className="dish-footer">
                        ...
                        <button
                          ...
                          onClick={(e) => { if (!isOutOfStock) handleQuickAdd(e, dish); }}
