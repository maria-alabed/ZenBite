...
import {
  ...
  SpiceChip,
  CrustChip,
} from "../components/product/ProductChips";
import {
  translateName,
...
export default function ProductDetails({ product, onClose }) {
  ...
  useEffect(() => {
    ...
    if (fromCart) {
      ...
      setSelectedSize(
        product.selectedSize || Object.keys(product.sizes || {})[0] || null,
      );
    } else {
      setQuantity(1);
      setSelectedAddons([]);
    ...
    const fetchSizes = async () => {
      try {
        const res = await fetch(
          `http://localhost:5000/api/sizes/${product.id}`,
        );
  ...
  useEffect(() => {
    ...
    const fetchAddons = async () => {
      try {
        const res = await fetch(
          `http://localhost:5000/api/addons/${product.category_id}`,
        );

        const json = await res.json();

        console.log("addons response:", json);
  ...
  const translatedDesc = translateDescription(
    originalName,
    product.description,
    t,
    language,
  ...
  const isPizza =
    product.category === "Italian Kitchen" && product.name?.includes("Pizza");
  const showSpice = spiceCategories.includes(product.category);
  const availAddons = addons;
  ...
  const basePrice = hasSizes
    ? Number(
        sizes.find((s) => s.size === selectedSize)?.price ?? product.price ?? 0,
      )
  ...
  const triggerFly = (buttonEl) => {
    ...
    const sx = rect.left + rect.width / 2 - 20,
      sy = rect.top + rect.height / 2 - 20;
    const ex = cartRect.left + cartRect.width / 2 - 20,
      ey = cartRect.top + cartRect.height / 2 - 20;
    el.style.left = sx + "px";
    el.style.top = sy + "px";
    el.style.setProperty("--start-x", sx + "px");
    el.style.setProperty("--start-y", sy + "px");
    el.style.setProperty("--end-x", ex + "px");
    el.style.setProperty("--end-y", ey + "px");
      ...
      >
        ...
        <div style={S.scrollArea}>
          ...
          <div style={S.content}>
              ...
              {hasSizes && (
                <div style={S.sizeRow}>
                  {sizes.map((sz) => (
                    <button
                      key={sz}
                      style={S.sizeBtn(selectedSize === sz)}
              ...
              <div style={S.addonsSummary}>
                ...
                {selectedAddons.map((a) => (
                  <span key={a.id} style={S.addonTag}>
                    {a.emoji}{" "}
                    {translateAddon(a.originalName || a.name, language)}
            ...
            <div style={{ marginBottom: 8 }}>
              <div style={{ ...S.sectionLabel, marginBottom: 6 }}>
        ...
        <div style={S.actionBar}>
          ...
          >
            ...
            <span style={{ fontSize: 17, fontWeight: 800, color: "#e07800" }}>
              ${Number(total || 0).toFixed(2)}{" "}
            </span>
          </div>
          {isOutOfStock ? (
            <button style={S.addBtnDis} disabled>
              ❌ {t.outOfStock}
            </button>
          ) : (
            <button
